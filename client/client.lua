...
RegisterNetEvent('unr3al_meth:client:drugged', function()
...
lib.callback.register('unr3al_meth:client:openContext', function(minigame, netId)
	...
	if failedSkillcheck then
		...
		TriggerServerEvent('unr3al_meth:server:blow', GetPlayerServerId(PlayerId()), netId)
	end
	return meth
end)

lib.callback.register('unr3al_meth:client:getMethType', function(netId)
	local entity = NetworkGetEntityFromNetworkId(netId)
	if not DoesEntityExist(entity) then
		return
	end
...
lib.callback.register('unr3al_meth:client:skillcheck', function(vehicle)
...
RegisterNetEvent('unr3al_meth:client:smoke', function(stop, netId)
	local entity = NetworkGetEntityFromNetworkId(netId)
	if not DoesEntityExist(entity) then
		return
	end
...
RegisterNetEvent('unr3al_meth:client:notify', function(notitype, message)
	notifications(notitype, message, Config.Noti.time)
end)

RegisterNetEvent('unr3al_meth:client:stop', function()
	if (Config.Cam) then
		toggleCam(false)
	end
...
RegisterNetEvent('unr3al_meth:client:blowup', function(posx, posy, posz, vehicle, netId)
	local entity = NetworkGetEntityFromNetworkId(netId)
...
RegisterNetEvent('unr3al_meth:client:registerContext', function()
	lib.registerContext({
		id = 'Event_01',
		title = Locales[Config.Locale]['Question_01'],
		onExit = function()
			Wait(20)
			TriggerServerEvent('unr3al_meth:server:stopProduction', GetPlayerServerId(PlayerId()), NetworkGetNetworkIdFromEntity(cache.vehicle))
		end,
		options = {
					...
					if not Questions.DisableAll and Questions.Question_02.Enabled then
						...
						elseif Questions.Question_02.DifficultyAnswer_1 == 1 then
							...
							if success then
								...
								TriggerEvent('unr3al_meth:client:drugged')
							end
...
if (Config.StartProduction.Key.Enabled) then
	RegisterInput("StartProduction", "Start meth production", "keyboard", Config.StartProduction.Key.StartKey, function()
		TriggerServerEvent('unr3al_meth:server:start', NetworkGetNetworkIdFromEntity(cache.vehicle))
	end)
end
